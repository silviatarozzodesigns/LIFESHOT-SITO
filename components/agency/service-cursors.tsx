 ...
 * separati desktop (6 servizi) e mobile (4). Con le slide fotografiche
 * attive i cursori si attenuano. Rispetta prefers-reduced-motion.
 */
...
const MOBILE: Chip[] = [
  { label: "Siti web", x: 24, y: 22, from: "left", delay: 0.3 },
  { label: "Grafiche", x: 72, y: 29, from: "right", delay: 0.65 },
  { label: "Social", x: 26, y: 77, from: "left", delay: 1.0 },
  { label: "Foto", x: 70, y: 84, from: "bottom", delay: 1.35 },
];
...
/** Puntatore stile cursore (freccia piena, giallo brand) */
function Pointer({ flipped }: { flipped: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden
      className={cn(
        "absolute -top-3 h-[18px] w-[18px] drop-shadow-[0_2px_6px_rgba(0,0,0,0.5)]",
        flipped ? "-right-2.5 -scale-x-100" : "-left-2.5"
      )}
    >
      <path
...
function CursorChip({ chip, index }: { chip: Chip; index: number }) {
  ...
  // Il puntatore sta sul lato della pillola rivolto verso il centro pagina
  const flipped = chip.x > 50;
